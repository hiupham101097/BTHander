...
const productBlank = { name: "", description: "", content: "", image_url: "", app_url: "", product_type: "trial", price: "0", currency: "VND", specifications: "", status: "published" };
...
export default function CatalogManager({ kind }) {
  ...
  const edit = (item) => {
    setEditing(item.id);
    setForm(product ? { name: item.name, description: item.description || "", content: item.content || "", image_url: item.image_url || "", app_url: item.app_url || "", product_type: item.product_type, price: String(item.price), currency: item.currency, specifications: item.specifications.join(", "), status: item.status } : { name: item.name, title: item.title, bio: item.bio || "", avatar_url: item.avatar_url || "", profile_intro: item.profile_intro || "", skills: (item.skills || []).join(", "), experience: (item.experience || []).join("\n"), featured_projects: (item.featured_projects || []).join("\n"), articles: (item.articles || []).join("\n"), sort_order: String(item.sort_order), status: item.status });
  ...
  const save = async (event) => {
    ...
    const payload = product ? { ...form, description: String(form.description || "").slice(0, 1000), content: String(form.content || ""), image_url: String(form.image_url || ""), app_url: String(form.app_url || ""), price: form.product_type === "trial" ? 0 : Number(form.price), currency: form.currency.toUpperCase(), specifications: form.specifications.split(",").map((v) => v.trim()).filter(Boolean) } : { ...form, sort_order: Number(form.sort_order), skills: form.skills.split(",").map((v) => v.trim()).filter(Boolean), experience: lines(form.experience), featured_projects: lines(form.featured_projects), articles: lines(form.articles) };
    setSaving(true);
    try {
  ...
  return <div>
    ...
    {open && <div className="admin-modal-backdrop" onMouseDown={() => setOpen(false)}><form className="admin-form admin-modal" onSubmit={save} onMouseDown={(e) => e.stopPropagation()}><div className="admin-form-header"><h3>{editing ? "Chỉnh sửa" : "Thêm mới"} {addLabel}</h3><button className="icon-btn" type="button" onClick={() => setOpen(false)}><X size={17} /></button></div>{error && <p className="form-error">{error}</p>}<div className="admin-form-grid">{product ? <><label>Tên ứng dụng/sản phẩm<input required value={form.name} onChange={update("name")} /></label><label>Nhóm hiển thị<select value={form.product_type} onChange={update("product_type")}><option value="trial">Ứng dụng miễn phí</option><option value="sale">Sản phẩm bán</option></select></label><div className="admin-form-full"><span className="form-label">Ảnh đại diện</span><ImageUpload value={form.image_url} onChange={(url) => update("image_url")({ target: { value: url } })} label="Tải ảnh sản phẩm" /></div><label className="admin-form-full">Mô tả ngắn <small>{form.description.length}/1000</small><textarea rows="3" maxLength="1000" value={form.description} onChange={update("description")} /></label><label className="admin-form-full">Nội dung chi tiết<textarea rows="6" value={form.content} onChange={update("content")} placeholder="Giới thiệu đầy đủ, cách sử dụng, điểm nổi bật…" /></label><label>Đặc điểm (phân cách bằng dấu phẩy)<input value={form.specifications} onChange={update("specifications")} /></label><label>Liên kết sử dụng/tải ứng dụng<input type="url" value={form.app_url} onChange={update("app_url")} placeholder="https://..." /></label><label>Giá<input required type="number" min="0" value={form.price} onChange={update("price")} disabled={form.product_type === "trial"} /></label><label>Trạng thái<select value={form.status} onChange={update("status")}><option value="published">Xuất bản</option><option value="draft">Bản nháp</option></select></label></> : <TeamFields form={form} update={update} editing={editing} />}</div><button className="btn-primary admin-add" disabled={saving}>{saving ? "Đang lưu…" : "Lưu và hiển thị"}</button></form></div>}
