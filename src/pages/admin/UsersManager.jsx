...
export default function UsersManager() {
  ...
  return <div><div className="admin-toolbar"><h2 className="admin-section-title">Tài khoản ({accounts.length})</h2></div><p className="admin-note">Tài khoản người dùng được tạo từ trang đăng ký. Bạn có thể thay đổi quyền tại đây.</p>{error && <p className="form-error">{error}</p>}<div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Tài khoản</th><th>Email</th><th>Quyền</th></tr></thead><tbody>{accounts.map((account) => <tr key={account.id}><td><div className="admin-table-item"><span className="admin-table-icon"><ShieldCheck size={15} /></span><strong>{account.name}</strong></div></td><td>{account.email}</td><td><select className="admin-select" value={account.role} onChange={(event) => changeRole(account.id, event.target.value)}><option value="user">Người dùng</option><option value="admin">Quản lý</option></select></td></tr>)}</tbody></table></div></div>;
}
